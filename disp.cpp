...
void showQRCode(double lat, double lng) {
  ...
  cfg.display_func = [](esp_qrcode_handle_t qrcode) {
    ...
    int x_offset = (128 - (size * scale)) / 2,
        y_offset = (64 - (size * scale)) / 2;
    ...
    for (int y = 0; y < size; y++)
      for (int x = 0; x < size; x++)
        if (esp_qrcode_get_module(qrcode, x, y)) {
          ...
          display.fillRect(x_offset + (x * scale), y_offset + (y * scale), scale, scale);
        }
  ...
  if (lat == 0.0 && lng == 0.0)
    esp_qrcode_generate(&cfg, "https://tinyurl.com/54fes5pz");
  else {
    char s[4 + 4 + 6 + 1 + 4 + 6 + 1];
    snprintf(s, sizeof s, "geo:%.6f,%.6f", lat, lng);
    esp_qrcode_generate(&cfg, s);
  }
}
